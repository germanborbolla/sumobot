...
class ReceptionistTest
  ...
  "Receptionist" should {
    ...
    "replace html entities with their characters" in {
      sut ! Message(currentTimeStamp, im.id, somebodyElse.id, "&amp; &lt; &gt;", None, None)
      val result = probe.expectMsgClass(classOf[IncomingMessage])
      result.canonicalText should be("& < >")
      result.addressedToUs should be(true)
    }

    "include the timestamp and thread timestamp on the message" in {
      val ts = currentTimeStamp
      sut ! Message(ts, im.id, somebodyElse.id, "&amp; &lt; &gt;", None, Some("parent"))
      val result = probe.expectMsgClass(classOf[IncomingMessage])
      result.canonicalText should be("& < >")
      result.addressedToUs should be(true)
      result.ts should be(ts)
      result.thread_ts should be(Some("parent"))
    }
  }
